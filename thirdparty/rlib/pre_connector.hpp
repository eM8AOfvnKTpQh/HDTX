...
diff_time(const struct timeval& end, const struct timeval& start) {
  int64_t diff =
      (end.tv_sec > start.tv_sec) ? (end.tv_sec - start.tv_sec) * 1000000L : 0;
...
class PreConnector {  // helper class used to exchange QP information using
                      // TCP/IP
 public:
  ...
  static std::string host_to_ip(const std::string& host) {
    ...
    if (cache.find(host) != cache.end()) return cache[host];
    ...
    res = std::string(ip);
    if (res != "") cache.insert(std::make_pair(host, res));
    return res;
  }
};
