...
typedef std::function<void(QP*)> connection_callback_t;
...
class RdmaCtrl {
 public:
   ...
   * if there is a previous call, return previous results unless clear_dev_info
   * has been called
   */
   ...
   * The get_device returns previously opened device of this thread, if it is
   * already opened
   */
  ...
  /**
   * The *callback* is called once a QP connection is established
   */
  ...
  /**
   * Each RDMA NIC has multiple ports, so we use two-dimeson index to locate the
   * target port. convert_port_idx provides a way to translate the one-dimeson
   * index to the two-dimeson
   */
   ...
   */
  bool register_memory(int id, const char* buf, uint64_t size,
                       RNicHandler* rnic,
   ...
   */
  RCQP* create_rc_qp(QPIdx idx, RNicHandler* dev,
                     MemoryAttr* local_attr = NULL);
  UDQP* create_ud_qp(QPIdx idx, RNicHandler* dev,
                     MemoryAttr* local_attr = NULL);
  ...
  std::map<uint64_t, QP*> get_rc_qps();
  std::map<uint64_t, QP*> get_rc_wait_qps();
