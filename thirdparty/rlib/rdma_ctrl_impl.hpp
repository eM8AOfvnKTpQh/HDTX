...
class RdmaCtrl::RdmaCtrlImpl {
 ...
 public:
  ...
  RCQP* create_rc_qp(QPIdx idx, RNicHandler* dev, MemoryAttr* attr,
                     std::map<uint64_t, QP*>& qps, int create_flags = 0) {
    RCQP* res = nullptr;
    {
      SCS s;
      uint64_t qid = get_rc_key(idx);
      if (qps.find(qid) != qps.end()) {
        ...
        qps.insert(std::make_pair(qid, res));
      }
    };
    return res;
  }
          ...
          case ConnArg::QP: {
            ...
            switch (arg.payload.qp.qp_type) {
              ...
              case IBV_QPT_RC: {
                ...
                RDMA_LOG(INFO)
                    << "received QP from client, node id: "
                ...
                if (arg.use_offload) {  // use offload
                  ...
                  self_qp->connect(wait_qp);

                  ((RCQP*)qp)->use_offload = true;
                  ((RCQP*)qp)->wait_qp = wait_qp;
                }
              } break;
              default:
                RDMA_LOG(ERROR)
            ...
            if (qp != nullptr) {
              ...
              reply.ack = SUCC;
              qp_callback_(qp);  // call the user callback
            }
 ...
 private:
  ...
  // wait QPs
  std::map<uint64_t, QP*> wait_qps_;

  // self-connect QPs
  std::map<uint64_t, QP*> self_qps_;
...
inline __attribute__((always_inline)) MemoryAttr RdmaCtrl::get_local_mr(
    int mr_id) {
