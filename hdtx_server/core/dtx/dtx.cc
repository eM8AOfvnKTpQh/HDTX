...
#if READ_LOCK
  if (!IssueReadLock(pending_hash_rw, pending_insert_off_rw, pending_lock_rw)) return false;
#else
...
void DTX::Abort() {
  if (global_meta_man->txn_system == DTX_SYS::HDTX) {
    ...
    for (auto &index : locked_rw_set) {
      ...
      // If transaction fails during validation and commit phases, make the read-write-set's items visable
