...
bool DTX::CompareIssueLocking(std::vector<Lock>& pending_lock) {
  for (auto& index : not_locked_rw_set) {
    ...
    *(lock_t*)cas_buf = 0xdeadbeaf;
    ...
    if (!coro_sched->RDMACAS(coro_id, qp, cas_buf, it->GetRemoteLockAddr(), STATE_CLEAN, u_id)) return false;
  }
  return true;
}
...
bool DTX::CompareIssueLockValidation(std::vector<ValidateRead>& pending_validate) {
  ...
  for (auto& index : not_locked_rw_set) {
    ...
    *(lock_t*)cas_buf = 0xdeadbeaf;
    ...
    node_id_t primary_node_id = global_meta_man->GetPrimaryNodeID(it->table_id);
    RCQP* qp = thread_qp_man->GetRemoteDataQPWithNodeID(primary_node_id);
    pending_validate.push_back(ValidateRead{.qp = qp, .item = &read_write_set[index], .lock_buf = cas_buf, .version_buf = version_buf, .has_lock_in_validate = true});
  ...
  for (auto& set_it : read_only_set) {
    ...
    RCQP* qp = thread_qp_man->GetRemoteDataQPWithNodeID(primary_node_id);
    char* version_buf = thread_rdma_buffer_alloc->Alloc(sizeof(version_t));
    pending_validate.push_back(ValidateRead{.qp = qp, .item = &set_it, .lock_buf = nullptr, .version_buf = version_buf, .has_lock_in_validate = false});
