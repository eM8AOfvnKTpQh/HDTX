...
std::vector<double> attemp_tp_vec;  // total throughput
std::vector<double> tp_vec;         // commit throughput
std::vector<double> medianlat_vec;  // 50th latency
...
void Handler::OutputResult(std::string bench_name, std::string system_name) {
  ...
  if (bench_name == "MICRO") {
    of_detail << total_attemp_tp << " " << total_tp << " " << medianlat_vec[0]
              ...
              << " " << avg_tail << " " << avg_avglat << " " << avg_sla << " "
              << " " << avg_commit << " " << avg_sample_avglat << " "
              << avg_sample_taillat << " " << avg_sample_commit << std::endl;

    of << system_name << " " << total_attemp_tp / 1000 << " " << total_tp / 1000
       << " " << avg_median << " " << avg_tail << " " << avg_avglat << " "
       << avg_sla << " " << avg_commit << " " << avg_sample_avglat << " "
       << avg_sample_taillat << " " << avg_sample_commit << std::endl;
  } else {
    of_detail << total_attemp_tp << " " << total_tp << " " << medianlat_vec[0]
              << " " << medianlat_vec[thread_num - 1] << " " << avg_median
  ...
  if (bench_name == "MICRO") {
    std::cerr << system_name << " " << total_attemp_tp / 1000 << " tp="
              << total_tp / 1000 << " mid="
              << " " << avg_median << " tail=" << avg_tail << " avg=" << avg_avglat
              << " sla=" << avg_sla << " com=" << avg_commit << " sa_avg=" << avg_sample_avglat
              << " sa_tail=" << avg_sample_taillat << " sa_com=" << avg_sample_commit
              << std::endl;
  } else {
    std::cerr << system_name << " " << total_attemp_tp / 1000 << " tp="
              << total_tp / 1000 << " mid="
              << " " << avg_median << " tail=" << avg_tail << " avg=" << avg_avglat
              << " sla=" << avg_sla << " com=" << avg_commit << std::endl;
  }
...
void Handler::GenThreadsForMICRO() {
  std::string config_filepath = "../../../config/compute_node_config.json";
  // std::string s = "sed -i '8c \"txn_system\": 4,' " + config_filepath;
  // system(s.c_str());
  auto json_config = JsonConfig::load_file(config_filepath);
  auto client_conf = json_config.get("local_compute_node");
  node_id_t machine_num = (node_id_t)client_conf.get("machine_num").get_int64();
