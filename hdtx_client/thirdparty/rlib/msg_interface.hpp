...
class MsgAdapter {
 public:
  MsgAdapter(msg_callback_t_ callback) : callback_(callback) {}

  MsgAdapter() {}

  void set_callback(msg_callback_t_ callback) { callback_ = callback; }
  ...
  virtual ConnStatus send_pending(int node_id, int tid, const char* msg,
                                  int len) {
   ...
   */
  virtual ConnStatus flush_pending() { return SUCC; }

  /**
   * Examples to use batching at the sender side
   * Broadcast the message to a set of servers
   */
  virtual ConnStatus broadcast_to(const std::set<int>& nodes, const char* msg,
                                  int len) {
    prepare_pending();
  ...
  virtual ConnStatus broadcast_to(int* nodes, int num, const char* msg,
                                  int len) {
    prepare_pending();
    for (int i = 0; i < num; ++i) {
   ...
   */
  virtual int msg_meta_len() { return 0; }
