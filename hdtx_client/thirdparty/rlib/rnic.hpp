...
struct RNicInfo {
  ...
  void query_port_infos(ibv_context* ctx) {
    ...
    if (!query_dev_attribute(ctx, attr)) return;
    ...
    for (uint port_id = 1; port_id <= attr.phys_port_cnt; ++port_id) {
      ...
      if (rc != 0) {
        RDMA_LOG(ERROR) << "query port_id " << port_id << " on device "
                        << dev_id << "error.";
        continue;
      }
      ...
      if (port_attr.phys_state != IBV_PORT_ACTIVE &&
        ...
        RDMA_LOG(WARNING) << "query port_id " << port_id << " on device "
                          << dev_id << " not active.";
        continue;
      }
      ...
      switch (port_attr.link_layer) {
        ...
        default:
          RDMA_LOG(WARNING)
              << "unknown link layer at this port: " << port_attr.link_layer;
          link_layer = "Unknown";
      };
  ...
  void query_active_gids(ibv_context* ctx) {
    ...
    if (rc != 0) {
      RDMA_LOG(WARNING) << "query port attribute at dev " << dev_name
                        << ",port " << port_id
  ...
  void print() const { RDMA_LOG(3) << to_string(); }

  std::string to_string() const {
    std::ostringstream oss;

    oss << "device " << dev_name << " has " << active_ports.size()
        << " active ports.";
...
struct RNicHandler {
  RNicHandler(int dev_id, int port_id, ibv_context* ctx, ibv_pd* pd, int lid,
              int gid = 0)
      : dev_id(dev_id),
        port_id(port_id),
        ctx(ctx),
  ...
  address_t query_addr(uint8_t gid_index) {
    ...
    address_t addr{.subnet_prefix = gid.global.subnet_prefix,
  ...
  ~RNicHandler() {
    ...
    RDMA_VERIFY(INFO, ibv_dealloc_pd(pd) == 0)
        << "failed to dealloc pd at device " << dev_id << "; w error "
        << strerror(errno);
  }
