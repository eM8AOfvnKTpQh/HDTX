...
inline uint64_t encode_qp_64b_id(int m, int idx) {
  return static_cast<uint64_t>((static_cast<uint64_t>(m) << 32) |
...
class QPImpl {
 public:
  ...
  static ConnStatus get_remote_helper(ConnArg* arg, ConnReply* reply,
  ...
  static ConnStatus poll_till_completion(ibv_cq* cq, ibv_wc& wc,
    ...
    int64_t numeric_timeout = (timeout.tv_sec == 0 && timeout.tv_usec == 0)
                                  ...
                                  : timeout.tv_sec * 1000000L + timeout.tv_usec;
    do {
      asm volatile("" ::: "memory");
...
class RCQPImpl {
 public:
  ...
  static const int RC_MAX_SEND_SIZE = 4096;
  static const int RC_MAX_RECV_SIZE = 4096;
  ...
  static void init(ibv_qp*& qp, ibv_cq*& cq, RNicHandler* rnic,
    ...
    RDMA_VERIFY(ERROR, cq != nullptr) << "create cq error: " << strerror(errno);

#ifdef EXP_VERBS
    struct ibv_exp_cq_attr cq_attr {};
    memset(&cq_attr, 0, sizeof(cq_attr));
    cq_attr.cq_cap_flags =
        IBV_EXP_CQ_IGNORE_OVERRUN;  // prevent the newly completed WCs from
                                    // being discarded
    cq_attr.comp_mask = IBV_EXP_CQ_ATTR_CQ_CAP_FLAGS;
    auto rc = ibv_exp_modify_cq(cq, &cq_attr, IBV_EXP_CQ_CAP_FLAGS);
    if (rc) {
      RDMA_LOG(ERROR) << "failed to modify CQ, error: " << strerror(errno);
    }
#endif
    ...
    // TODO, need seperate handling for two-sided over RC QP
    ...
    qp_init_attr.cap.max_send_sge = 6;
    qp_init_attr.cap.max_recv_sge = 6;
    qp_init_attr.cap.max_inline_data = MAX_INLINE_SIZE;

    qp_init_attr.exp_create_flags =
        IBV_EXP_QP_CREATE_CROSS_CHANNEL |  // used for WAIT wr
        IBV_EXP_QP_CREATE_IGNORE_SQ_OVERFLOW |
        IBV_EXP_QP_CREATE_IGNORE_RQ_OVERFLOW;

    // IBV_EXP_QP_CREATE_MANAGED_SEND: used for ENABLE wr
    qp_init_attr.exp_create_flags |= create_flags;

    qp_init_attr.comp_mask =
        IBV_EXP_QP_INIT_ATTR_CREATE_FLAGS | IBV_EXP_QP_INIT_ATTR_PD;

    qp_init_attr.pd = rnic->pd;

    qp = ibv_exp_create_qp(rnic->ctx, &qp_init_attr);
#else
...
class UDQPImpl {
 public:
  ...
  static void init(ibv_qp*& qp, ibv_cq*& cq, ibv_cq*& recv_cq,
                   RNicHandler* rnic) {
    ...
    if ((cq = ibv_create_cq(rnic->ctx, config.max_send_size, nullptr, nullptr,
                            0)) == nullptr) {
      RDMA_LOG(ERROR) << "create send cq for UD QP error: " << strerror(errno);
      return;
    }

    if ((recv_cq = ibv_create_cq(rnic->ctx, config.max_recv_size, nullptr,
                                 nullptr, 0)) == nullptr) {
    ...
    if (!ready2rcv(qp, rnic)) {
      RDMA_LOG(WARNING) << "change ud qp to ready to recv error: "
                        << strerror(errno);
    }
    if (!ready2send(qp, config)) {
      RDMA_LOG(WARNING) << "change ud qp to ready to send error: "
                        << strerror(errno);
    }
  }

  /**
   * Unlike RC, which change status happens at different places, so F, the
   * function which generates configurations, are passed as templates. On the
   * other hand, UD change status at the same time. So it is more convenient to
   * passed the configuration generated by the F to the functions.
   */
