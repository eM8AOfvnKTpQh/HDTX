...
struct DataItem {
  ...
  ALWAYS_INLINE
  size_t GetSerializeSize() const { return sizeof(*this); }
  ...
  uint64_t GetRemoteLockAddr() {
    return remote_offset + sizeof(table_id) + sizeof(value_size) + sizeof(key) +
           sizeof(remote_offset) + sizeof(version) + MAX_ITEM_SIZE +
           sizeof(valid) + sizeof(user_insert);
  }
  ...
  uint64_t GetRemoteLockAddr(offset_t remote_item_off) {
    return remote_item_off + sizeof(table_id) + sizeof(value_size) +
           ...
           MAX_ITEM_SIZE + sizeof(valid) + sizeof(user_insert);
  }
  ...
  void Debug(t_id_t tid) const {
    // For debug usage
    TLOG(INFO, tid) << "[Item debug] table id: " << this->table_id
                    << ", value size: " << this->value_size
                    << ", key: " << this->key
                    << ", remote offset: " << this->remote_offset
                    << ", version: " << this->version
                    << ", lock: " << (int)this->lock
                    << ", valid: " << (int)this->valid
                    << ", user insert: " << (int)this->user_insert << std::endl;
  ...
  void Debug() const {
    // For debug usage
    RDMA_LOG(DBG) << "[Item debug] table id: " << this->table_id
                  << ", value size: " << this->value_size
                  << ", key: " << this->key
                  << ", remote offset: " << this->remote_offset
                  << ", version: " << this->version << ", lock: " << std::hex
                  << "0x" << this->lock << ", valid: " << std::dec
                  << (int)this->valid
                  << ", user insert: " << (int)this->user_insert;
  }
} Aligned8;  // Size: 560B in X86 arch.

const size_t DataItemSize = sizeof(DataItem);

// The size of RDMA read, that is after write to emulate rdma flush
const size_t RFlushReadSize = 1;
