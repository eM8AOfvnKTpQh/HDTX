...
class HashStore {
 public:
  HashStore(table_id_t table_id, uint64_t bucket_num, MemStoreAllocParam* param)
    ...
    RDMA_LOG(INFO) << "Table " << table_id
                   << " size: " << table_size / 1024 / 1024
                   << " MB. Start address: " << std::hex << "0x"
                   << (uint64_t)data_ptr << ", base_off: 0x" << base_off
                   << ", bucket_size: " << std::dec
                   << ITEM_NUM_PER_NODE * DataItemSize << " B";
 ...
 private:
  ...
  // Start of the memory region address, for installing remote offset for data
  // item
...
DataItem* HashStore::LocalInsert(itemkey_t key, const DataItem& data_item,
  ...
  RDMA_LOG(INFO) << "Table " << table_id
                 ...
                 << ". Current slotnum/bucket: " << ITEM_NUM_PER_NODE;
