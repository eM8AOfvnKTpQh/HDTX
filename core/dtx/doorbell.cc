...
void LockReadBatch::SetLockReq(char *local_addr, uint64_t remote_off,
  ...
  sr[0].wr.atomic.compare_add = compare_add;
...
void WriteUnlockBatch::SetWriteReq(char *local_addr, uint64_t remote_off,
                                   size_t size) {
...
void WriteUnlockBatch::SetUnLockReq(char *local_addr, uint64_t remote_off,
  ...
  sr[1].wr.atomic.compare_add = compare_add;
...
bool WriteUnlockBatch::SendReqs(CoroutineScheduler *coro_sched, RCQP *qp,
  ...
  if (use_atomic) {
    if (use_off) sr[1].wr.atomic.remote_addr += qp->remote_mr_.buf;
    sr[1].wr.atomic.rkey = qp->remote_mr_.key;
  } else {
    if (use_off) sr[1].wr.rdma.remote_addr += qp->remote_mr_.buf;
    sr[1].wr.rdma.rkey = qp->remote_mr_.key;
  }
