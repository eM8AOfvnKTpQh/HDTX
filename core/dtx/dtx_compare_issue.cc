  ...
  for (auto &item : read_only_set) {
    ...
    if (global_meta_man->txn_system == DTX_SYS::DrTMH) {
      ...
      if (offset != NOT_FOUND) {
        ...
        if (!coro_sched->RDMARead(coro_id, qp, data_buf, offset,
                                  DataItemSize)) {
          return false;
        }
        continue;
  ...
  for (size_t i = 0; i < read_write_set.size(); i++) {
    ...
    if (global_meta_man->txn_system == DTX_SYS::DrTMH) {
      ...
      if (offset != NOT_FOUND) {
        it->remote_offset = offset;
        char *data_buf = thread_rdma_buffer_alloc->Alloc(DataItemSize);
        pending_direct_rw.emplace_back(
            DirectRead{.qp = qp,
...
bool DTX::CompareIssueLocking(std::vector<Lock> &pending_lock) {
  for (auto &index : not_locked_rw_set) {
    ...
    *(lock_t *)cas_buf = 0xdeadbeaf;
    ...
    pending_lock.push_back(Lock{.qp = qp,
                                .item = &read_write_set[index],
                                .cas_buf = cas_buf,
                                .lock_off = it->GetRemoteLockAddr()});
    if (!coro_sched->RDMACAS(coro_id, qp, cas_buf, it->GetRemoteLockAddr(),
                             STATE_CLEAN, encode_id(t_id, coro_id)))
      return false;
  }
  return true;
}
...
bool DTX::CompareIssueValidation(std::vector<Version> &pending_version_read) {
  for (auto &set_it : read_write_set) {
    ...
    pending_version_read.push_back(
        Version{.item = &set_it, .version_buf = version_buf});
...
bool DTX::CompareIssueLockValidation(
...
bool DTX::CompareIssueCommitBackup() {
  ...
  *((t_id_t *)header) = t_id;
  ...
  for (auto &set_it : read_write_set) {
    auto it = set_it.item_ptr;
    if (!it->user_insert)
      it->version = tx_id;  // Maintain the version that user specified
    it->lock = STATE_CLEAN;
    const std::vector<node_id_t> *backup_node_ids =
        global_meta_man->GetBackupNodeID(it->table_id);
    for (auto node_id : *backup_node_ids) {
      ...
      offset_t log_offset =
          thread_remote_log_offset_alloc->GetNextLogOffset(node_id, total_size);
      auto &remote_log_mr =
          thread_qp_man->GetRemoteLogQPWithNodeID(node_id)->remote_mr_;
      auto &local_data_mr = data_qp->local_mr_;
      // We need to use data_qp to write redo logs to backups, because log write
      // is on the critical path. In this way, data_qp will be recorded in the
      // coroutine's pending_counts, and the coro_sched will yield correctly.
      // But we need to write logs, so we have to use log qp to get the remote
      // log region's attr.
      if (!coro_sched->RDMAWrite(coro_id, data_qp, log_buffer, log_offset,
                                 total_size, local_data_mr, remote_log_mr))
        return false;
    }
  }
  ...
  *((t_id_t *)header) = t_id;
  ...
  for (auto &set_it : read_write_set) {
    ...
    for (auto node_id : *backup_node_ids) {
      ...
      // RDMA FLUSH
      char *flush_buffer =
          (char *)thread_rdma_buffer_alloc->Alloc(RFlushReadSize);
      if (!coro_sched->RDMARead(coro_id, data_qp, flush_buffer, log_offset,
                                RFlushReadSize))
        return false;
    }
  }
...
bool DTX::CompareIssueCommitBackupSelectiveFlush() {
  ...
  ssize_t current_i = 0;
  for (auto &set_it : read_write_set) {
    // Each read-write data needs to identify which machine is its backup
    // machine
    auto it = set_it.item_ptr;
    if (!it->user_insert) it->version = tx_id;
    const std::vector<node_id_t> *backup_node_ids =
    ...
    for (auto node_id : *backup_node_ids) {
      ...
      // is on the critical path. In this way, data_qp will be recorded in the
      // coroutine's pending_counts, and the coro_sched will yield correctly.
      // Since we need to write logs, we have to use log qp to get the remote
      // log region's attr.
      if (!coro_sched->RDMAWrite(coro_id, data_qp, log_buffer, log_offset,
                                 total_size, local_data_mr, remote_log_mr))
        return false;
  ...
  for (auto &set_it : read_write_set) {
    ...
    for (auto node_id : *backup_node_ids) {
      ...
      if (current_i == read_write_set.size() - 1) {
        ...
        std::shared_ptr<WriteFlushBatch> doorbell =
            std::make_shared<WriteFlushBatch>();
        ...
        if (!doorbell->SendReqs(coro_sched, data_qp, coro_id, remote_log_mr))
          return false;
      } else {
        if (!coro_sched->RDMAWrite(coro_id, data_qp, log_buffer, log_offset,
                                   total_size, local_data_mr, remote_log_mr))
...
bool DTX::CompareIssueCommitPrimary() {
  for (auto &set_it : read_write_set) {
    ...
    it->lock = encode_id(t_id, coro_id);  // version has been added during commit backup

    std::shared_ptr<WriteUnlockBatch> doorbell =
        std::make_shared<WriteUnlockBatch>();
    doorbell->SetWriteReq(data_buf, it->remote_offset, DataItemSize);

    *(lock_t *)cas_buf = STATE_CLEAN;
    doorbell->SetUnLockReq(cas_buf, it->GetRemoteLockAddr());
    if (!doorbell->SendReqs(coro_sched, qp, coro_id, true, false)) return false;

#if RFLUSH == 1
    // RDMA full FLUSH. Selective flush does not open this
    char *flush_buffer =
        (char *)thread_rdma_buffer_alloc->Alloc(RFlushReadSize);
    if (!coro_sched->RDMARead(coro_id, qp, flush_buffer, it->remote_offset,
                              RFlushReadSize))
      return false;
#endif
  }
...
bool DTX::CompareIssueTruncate() {
  for (auto &set_it : read_write_set) {
    ...
    for (size_t i = 0; i < backup_node_ids->size(); i++) {
      ...
      auto rc = coro_sched->RDMAWrite(coro_id, backup_qp, data_buf,
                                      remote_item_off, DataItemSize);
      if (rc != true) {
        RDMA_LOG(INFO) << "Thread " << t_id << " , Coroutine " << coro_id
                       << " truncate fails";
        return false;
      }
    }
