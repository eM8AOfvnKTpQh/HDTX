      ...
      if (likely(fetched_item->valid)) {
        *it = *fetched_item;
        ...
        if (unlikely((it->lock & STATE_INVISIBLE))) {
          ...
          // RDMA_LOG(DBG) << "Invisible in CheckDirectRO, lock=" << std::hex <<
          // it->lock << ", table_id=" << it->table_id << ", key=" << std::dec
          // << it->key << ", tx_id=" << tx_id;
  ...
  for (auto& res : pending_hash_ro) {
    ...
    if (likely(find)) {
      ...
      if (unlikely((it->lock & STATE_INVISIBLE))) {
        ...
        pending_invisible_ro.emplace_back(
            InvisibleRead{.qp = res.qp, .buf = cas_buf, .off = lock_offset});
        // RDMA_LOG(DBG) << "Invisible in CheckHashRO, lock=" << std::hex <<
        // it->lock << ", table_id=" << it->table_id << ", key=" << std::dec <<
        // it->key << ", tx_id=" << tx_id;
        if (!coro_sched->RDMARead(coro_id, res.qp, cas_buf, lock_offset,
                                  sizeof(lock_t)))
...
bool DTX::CheckInvisibleRO(std::list<InvisibleRead>& pending_invisible_ro) {
  ...
  for (auto iter = pending_invisible_ro.begin();
    ...
    while (lock_value & STATE_INVISIBLE) {
      ...
      // lock_value;
      auto rc = res.qp->post_send(IBV_WR_RDMA_READ, res.buf, sizeof(lock_t),
                                  res.off, IBV_SEND_SIGNALED);
      if (rc != SUCC) {
        TLOG(ERROR, t_id) << "client: post read (read visible flag) fail. rc="
                          << rc;
        exit(-1);
      }
      re_read++;
      ...
      if (rc != SUCC) {
        TLOG(ERROR, t_id) << "client: poll read (read visible flag) fail. rc="
                          << rc;
        exit(-1);
      }
