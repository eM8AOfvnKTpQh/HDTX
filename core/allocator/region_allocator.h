...
// This allocator is a global one which manages all the RDMA regions in this
// machine
...
class RDMARegionAllocator {
 public:
  RDMARegionAllocator(MetaManager* global_meta_man,
                      t_id_t thread_num_per_machine) {
    size_t global_mr_size =
        (size_t)thread_num_per_machine * PER_THREAD_ALLOC_SIZE;
    // Register a buffer to the previous opened device. It's DRAM in compute
    // pools
    ...
    RDMA_ASSERT(global_meta_man->global_rdma_ctrl->register_memory(
        CLIENT_MR_ID, global_mr, global_mr_size, global_meta_man->opened_rnic));
  }
  ...
  std::pair<char*, char*> GetThreadLocalRegion(t_id_t tid) {
    ...
    return std::make_pair(global_mr + tid * PER_THREAD_ALLOC_SIZE,
                          global_mr + (tid + 1) * PER_THREAD_ALLOC_SIZE);
  }
