...
class RDMABufferAllocator {
 public:
  ...
  char* Alloc(size_t size) {
    ...
    // can be re-used (i.e., overwritten) at the front offset, i.e., 0. This is
    // almost always true, because the local region is typically GB-scale, and
    // hence the front allocated buffer has already finished serving for RDMA
    // requests and replies, or has already aborted. As such, our Allocator is
    // extremely fast due to simply moving the pointer. If anyone relies on a
    // more reliable allocator, you can just re-implement this Alloc interface
