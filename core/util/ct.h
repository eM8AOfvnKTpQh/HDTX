...
class CT {
  static_assert(std::is_copy_constructible<K>::value, "K requires copy constructor");
  static_assert(std::is_copy_constructible<V>::value, "V requires copy constructor");
 ...
 public:
  CT() : power_of_2_(1), size_(0), hash_func_(Hash()) {
  ...
  CT(const CT& other) = delete;
  CT(CT&& other) = delete;
  CT& operator=(const CT& other) = delete;
  CT& operator=(CT&& other) = delete;
...
Reclaimer::HazardPointerList CT<K, V, Hash>::global_hp_list_;
...
template <typename K, typename V, typename Hash>
size_t CT<K, V, Hash>::reverse8bits_[256] = {R6(0), R6(2), R6(1),
                                             R6(3)};

template <typename K, typename V, typename Hash>
typename CT<K, V, Hash>::DummyNode*
CT<K, V, Hash>::InitializeBucket(BucketIndex bucket_index) {
...
template <typename K, typename V, typename Hash>
typename CT<K, V, Hash>::DummyNode*
CT<K, V, Hash>::GetBucketHeadByIndex(BucketIndex bucket_index) {
  int level = 1;
...
template <typename K, typename V, typename Hash>
bool CT<K, V, Hash>::InsertDummyNode(DummyNode* parent_head,
                                     DummyNode* new_head,
                                     DummyNode** real_head) {
  Node* prev;
  Node* cur;
...
template <typename K, typename V, typename Hash>
bool CT<K, V, Hash>::InsertRegularNode(DummyNode* head,
...
template <typename K, typename V, typename Hash>
bool CT<K, V, Hash>::SearchNode(DummyNode* head,
                                Node* search_node,
...
try_again:
  ...
  while (true) {
    ...
    if (is_marked_reference(next)) {
      ...
      reclaimer.ReclaimLater(cur, CT<K, V, Hash>::OnDeleteNode);
      reclaimer.ReclaimNoHazardPointer();
...
template <typename K, typename V, typename Hash>
bool CT<K, V, Hash>::DeleteNode(DummyNode* head,
                                Node* delete_node) {
  Node* prev;
  Node* cur;
  Node* next;
