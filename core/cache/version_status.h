...
// Assumption: In the future, there is one compute pool and one memory pool.
// Each pool contains many units intead of standalone machines. Therefore, the
// compute units can share some local caches, which include a update_status
// cache here
...
// It is an optimization validation. Therefore it is ok to set a large hash
// table with ideal concurrency and one bucket has one slot, which stores a k-v
// pair
struct StatusSlot {
  itemkey_t key;
...
class VersionCache {
 public:
  VersionCache() {
    ...
    for (int i = 0; i < MAX_TABLE_NUM; i++) {
      ...
      RDMA_LOG(INFO) << "Initializing table " << i << " " << sz / 1024 / 1024
                     << " MB";
  ...
  // Note that for the read-write set, it must be locked. And the validation is
  // done along with that locking anyway. So we don't care about the read-write
  // set True: a data version has been modified False: no data version changed
  VersionStatus CheckVersion(std::vector<DataSetItem>& read_only_set,
                             tx_id_t my_tx_id) {
    // If no conflict in SetVersion, then no conflict in CheckVersion, because
    // if a ro key has not occur, then it must not be updated by any other
    // writers, so its version remains unchanged. Hence, we only need to ensure
    // no conflict occurs in SetVersion
#if !TWO_HASH
 ...
 private:
  uint64_t GetBktId(itemkey_t t_k) { return (std_hash(t_k) % NUM_BKT); }
