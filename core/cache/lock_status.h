...
// Unfortunately, we find that in a high concurrency environment, substantial
// CPU CAS operations need to frequently retry to find empty slots due to the
// high collision rates in a small hash table. This will in turn cause the
// latency of local locking to be an order of magnitude higher than the remote
// locking, i.e., by using the microsecond-latency RDMA CAS. Due to this reason,
// we disable the use of local cache in FORD.
    ...
    for (auto& item : read_write_set) {
      ...
      for (uint64_t bkt_id = GetBktId(my_key);; bkt_id++) {
        ...
        if (probed_key == my_key) {
          ...
          bool exchanged = table[bkt_id].lock.compare_exchange_strong(
              expect_lock, STATE_LOCKED);
          if (!exchanged)
            return false;  // This key is locked by another coordinator
          ...
          bool exchanged =
              table[bkt_id].key.compare_exchange_strong(expect_key, my_key);
          if (exchanged) {
            // We cannot just use store here because another thread may cas succ
            // in `if (probed_key == my_key)' above after we fill the key. So we
            // need to do cas instead of pure store. Only cas succ we can get
            // the lock
            lock_t expect_lock = 0;
            bool exchanged = table[bkt_id].lock.compare_exchange_strong(
                expect_lock, STATE_LOCKED);
